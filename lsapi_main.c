...
static int  max_cmd_script_path_len = 45;
...
#else
static char *sapi_lsapi_getenv(const char * name, size_t name_len TSRMLS_DC)
#endif
{
    if ( lsapi_mode ) {
...
static int sapi_lsapi_send_headers_like_cgi(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
...
static int sapi_lsapi_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
    ...
    if ( mod_lsapi_mode ) {
        /* mod_lsapi mode */
        return sapi_lsapi_send_headers_like_cgi(sapi_headers TSRMLS_CC);
    }
...
static int lsapi_execute_script(TSRMLS_D)
{
    ...
    *p++ = ':';
    ...
    if (len > max_cmd_script_path_len - 1)
        len = len - max_cmd_script_path_len - 1;
    else
        len = 0;
    memccpy(p, SG(request_info).path_translated + len, 0, max_cmd_script_path_len);
...
#else
static int do_clean_shutdown = 0;
#endif
...
static int lsapi_module_main(int show_source TSRMLS_DC)
{
    ...
    zend_try {
        ...
        memset(argv0, 0, max_cmd_script_path_len);
    } zend_end_try();
    return 0;
}
...
static int cli_main( int argc, char * argv[] )
{
    ...
    zend_first_try     {
        ...
        if ( ret == -1 ) {
            if ( *p ) {
                zend_file_handle file_handle;
                zend_stream_init_fp(&file_handle, VCWD_FOPEN(*p, "rb"), *p);
#if PHP_MAJOR_VERSION > 8 || (PHP_MAJOR_VERSION == 8 && PHP_MINOR_VERSION >= 1)
                file_handle.primary_script = true;
#endif
...
static PHP_MINIT_FUNCTION(litespeed)
{
    ...
    p = getenv("LSAPI_MAX_CMD_SCRIPT_PATH_LEN");
    if (p) {
        int len = atoi(p);
        if (len > 0 && len < 256)
            max_cmd_script_path_len = len;
    }
    /*
...
PHP_FUNCTION(apache_get_modules)
{
    ...
    while( *name )
    {
        add_next_index_string(return_value, (char *)*name
#if PHP_MAJOR_VERSION < 7
                                        , 1
#endif
        );
